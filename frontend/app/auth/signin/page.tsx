...
export default function LoginPage() {
  ...
  // If wallet is connected → auto-login
  useEffect(() => {
    if (wallet && tgUser) {
      const loginPayload = {
  ...
  }, [wallet, tgUser, router]);

  return (
    <div className="min-h-screen bg-[#172a46] flex items-center justify-center p-6">
      <div className="bg-[#0f1e33] p-10 rounded-2xl shadow-2xl w-full max-w-lg text-center">
        ...
        <p className="text-gray-400 mt-3 mb-8">
          Login using your TON Wallet.  
          Telegram username will auto-link your account.
        </p>

        <button
          className="w-full bg-[#2a4562] hover:bg-[#3a5572] text-white py-3 px-5 rounded-xl text-lg font-semibold"
        ...
        <p className="text-gray-500 mt-4 text-sm">
          Logged in as: <span className="text-white font-semibold">{tgUser?.username}</span>
        </p>
      </div>
    </div>
  );
